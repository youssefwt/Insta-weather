...
const useWeather = (): {
  ...
  const { data, error: weatherError,isLoading,isValidating } = useSWR<WeatherData>(
    coordinates ? `http://api.weatherapi.com/v1/forecast.json?key=cca4405803f448dcab2151237231811&days=14&q=${coordinates.latitude},${coordinates.longitude}` : null,fetcher
  );
